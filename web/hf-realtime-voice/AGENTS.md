...
- Feed the exact generated playback PCM into the capture worklet as a non-audible reference; never substitute the static clone recording. Off is the default and preserves capture PCM. Adaptive uses conservative frame correlation and buffers/replays a confirmed barge-in; Strict suspends upload through the echo tail.
...
- Adaptive echo guard confirms uncorrelated double-talk for about 80 ms, buffers and replays the accepted onset in order, and resets reference state on Stop or replacement. The response-length setting is stored locally and sent as `max_response_tokens` with the next/local pipeline update; it does not change transcript or history behavior.
