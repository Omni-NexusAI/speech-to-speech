...
- Browser echo protection defaults to Off (native browser AEC only), so direct-audio PCM is never rewritten by custom DSP. Adaptive and Strict remain explicit opt-in choices.
