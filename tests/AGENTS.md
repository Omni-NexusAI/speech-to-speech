...
- Realtime tests should cover instruction acknowledgement, exact historical `input_audio` serialization for transcript-absent turns, final-only context commits, content-free trim counters, backend runtime identity, and PowerShell 5.1 lifecycle ordering.
- Direct-audio regression coverage must prove every accepted turn retains exactly one semantic user anchor before assistant/function-call/output state. Validated transcript text upgrades that anchor in place; absent or rejected metadata retains `input_audio`. A newer cumulative revision in the same session/turn replaces that anchor without duplicating history, stale cleanup cannot release its newer owner, and reused turn IDs in another session cannot alias it. Transcript metadata never starts a second request or gates a response. An explicit post-tool `response.create` still starts one follow-up, and missing transcripts retain their function-call/output pair without inserting placeholder user text.
