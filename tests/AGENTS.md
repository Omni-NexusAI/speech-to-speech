...
- Model-operation tests must prove serialization, busy-preview dropping, active transport closure, two-second-style detachment, and stale-release isolation. Browser DSP tests must cover Adaptive v3 coupled and uncoupled paths, Strict, Off, Bluetooth-scale delay, attenuation/reverberation/clipping, predictor reset, echo-only suppression, AEC-clean headsets, quieter correlated double-talk, 450 ms confirmation with brief natural speech gaps, and untouched PCM replay.
