...
- Model-operation tests must prove serialization, busy-preview dropping, active transport closure, two-second-style detachment, and stale-release isolation. Browser DSP tests must cover Adaptive v2, Strict, Off, Bluetooth-scale delay, attenuation/reverberation/clipping, predictor reset, echo-only suppression, 450 ms double-talk confirmation, and untouched PCM replay.
